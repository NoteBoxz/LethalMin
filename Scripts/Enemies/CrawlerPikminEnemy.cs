...
namespace LethalMin
{
    public class CrawlerPikminEnemy : PikminEnemy
    {
        List<NetworkObjectReference> PikminRefs = new List<NetworkObjectReference>();
        CrawlerAI crawlerAI = null!;
        float CheckInterval = 0.25f;
        float BiteCooldown = 1.5f;
        public float BiteResetCooldown = 2.0f;
        public int BiteLimmit = 3;

        protected override void Start()
        {
            base.Start();
            crawlerAI = enemyScript as CrawlerAI ?? throw new System.Exception("CrawlerPE: enemyScript is not a CrawlerAI");
            if (crawlerAI == null)
            {
                enabled = false;
            }
        ...
        void LateUpdate()
        {
            ...
            if (LethalMin.UseConfigsForEnemies)
            {
                BiteResetCooldown = LethalMin.Crawler_BiteCooldown.InternalValue;
                BiteLimmit = LethalMin.Crawler_BiteLimit.InternalValue;
            }
            ...
            {
                DoCheckInterval();
                CheckInterval = crawlerAI.AIIntervalTime + Random.Range(-0.015f, 0.015f);
            }
        }
        ...
        public void BiteNearbyPikmin(List<PikminAI> Pikmins)
        {
            ...
            {
                pikminAI.DeathSnapToPos = crawlerAI.mouthTarget;
                pikminAI.OverrideDelay = 0.75f;
                pikminAI.HitEnemy(2);
            }
            if (!crawlerAI.inSpecialAnimation)
            {
                crawlerAI.creatureAnimator.SetTrigger("HitPlayer");
            }
            crawlerAI.creatureVoice.PlayOneShot(crawlerAI.bitePlayerSFX);
        }
    }
}
