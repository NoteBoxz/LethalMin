...
ConfigUseString = r"(Generated Useable Variables GoES HERE\n)"
...
def ConstructConfigItemCode(item: ConfigItem):
    ...
    ConfigEventsToAdd.append(f'{item.InternalName}Config.OnValueChanged += (_, _) => {item.InternalName} = {item.InternalName}Config.Value;')
...
for item in ConfigUseVarsToAdd:
    new_content += re.sub(ConfigUseString, r"\1" +"\n" +  item, content, count=1)
