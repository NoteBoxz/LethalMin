...
namespace LethalMin.Compats
{
    ...
    public static class PiggyElevatorSystemPatch
    {
        ...
        [HarmonyPatch(nameof(ElevatorSystem.OnNetworkSpawn))]
        [HarmonyPostfix]
        public static void OnNetworkSpawnPostfix(ElevatorSystem __instance)
        {
            try
            {
            ...
            {
                LethalMin.Logger.LogError($"Failed to patch ElevatorSystem.Start due to: {e}");
            }
        }
